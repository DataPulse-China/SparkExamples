 ...
 * }}}
 * 一个连通分量算法示例。连通分量算法用其编号最小的顶点的 ID 标记图中的每个连通分量。例如，在社交网络中，连接的组件可以近似集群。
 * GraphX 在 [`ConnectedComponents` 对象][ConnectedComponents] 中包含算法的实现，我们计算示例社交网络数据集的连通分量。使用 {{{ binrun-example graphx.ConnectedComponentsExample }}} 运行
 */
